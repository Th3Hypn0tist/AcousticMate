...
class CombinedAcousticField {
  constructor({ domain, modes = [], speakers = [], speakerSets = [], frequencyRange = [20, 200], crossoverNetwork = null, q = 18, speedOfSound = 343 } = {}) {
  ...
  setDomain(domain) {
    ...
    this.roomField.setDimensions(domain.dimensions).setOpenings(typeof domain.openings === 'function' ? domain.openings() : []).setAcousticObjects(domain.acousticObjects ?? []);
    return this;
  }
}
