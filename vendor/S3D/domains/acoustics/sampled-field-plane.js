...
class SampledFieldPlane extends ScalarFieldView {
  ...
  recolor() { if (!this.samples.length) { this.range = this.valueRange ? [...this.valueRange] : [...this.sampleRange]; return this; } const [low, high] = this.valueRange ?? this.sampleRange; const span = Math.max(1e-12, high - low); for (const sample of this.samples) sample.color = this.color((sample.value - low) / span, sample.value, low, high); this.range = [low, high]; return this; }
  rebuild() { const [nx, nz] = this.resolution.map(Number); if (!Number.isInteger(nx) || !Number.isInteger(nz) || nx < 2 || nz < 2) throw new Error('SampledFieldPlane resolution requires two integers >= 2'); const { min, max } = this.bounds; const dx = (max[0] - min[0]) / nx; const dz = (max[2] - min[2]) / nz; const raw = []; let low = Infinity; let high = -Infinity; for (let iz = 0; iz < nz; iz++) for (let ix = 0; ix < nx; ix++) { const x = min[0] + (ix + .5) * dx; const z = min[2] + (iz + .5) * dz; const value = Number(this.sample(x, min[1], z)); const finite = Number.isFinite(value) ? value : 0; low = Math.min(low, finite); high = Math.max(high, finite); raw.push({ x, z, value: finite }); } this.samples = raw.map(({ x, z, value }) => ({ position: [x, min[1], z], scale: [dx * .49, this.height, dz * .49], color: [0, 0, 0], value })); this.sampleRange = [low, high]; this.recolor(); this.dirty = false; }
