...
function anchor(value = {}) {
  ...
  return { type, target: value.target ?? null, t: value.t == null ? null : finite(value.t, 'Anchor t'), position: value.position == null ? null : [finite(value.position[0], 'Anchor x'), finite(value.position[1], 'Anchor z')] };
}
class Measurement {
  constructor({ id, anchors, value, confidence = 1, source = 'measured', unit = 'm' } = {}) {
    ...
    this.id = id; this.type = 'distance'; this.anchors = anchors.map(anchor); this.value = finite(value, 'Measurement value');
...
function polygonEdges(polygon) {
  const vertices = polygon?.vertices ?? [], count = vertices.length;
  if (count < 2) return [];
  const edgeCount = polygon?.closed ? count : count - 1;
  return Array.from({ length: edgeCount }, (_, index) => {
    const start = vertices[index], end = vertices[(index + 1) % count];
    return { id: `edge:${start.id}:${end.id}`, start, end };
  });
}
function resolveAnchor(anchorValue, polygon) {
  const value = anchor(anchorValue), vertices = polygon?.vertices ?? [];
  if (value.type === 'free') { if (!value.position) throw new Error('Free measurement anchor requires position'); return [...value.position]; }
  if (value.type === 'vertex') { const vertex = vertices.find(item => item.id === value.target); if (!vertex) throw new Error(`Unknown measurement vertex anchor: ${value.target}`); return [vertex.x, vertex.z]; }
